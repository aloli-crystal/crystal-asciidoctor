...
describe Asciicrystal::Reader do
  ...
  context "With empty data" do
    it "has_more_lines? should return false with empty data" do
      Asciicrystal::Reader.new.has_more_lines?.should be_falsey
    end

    it "empty? should return true with empty data" do
      Asciicrystal::Reader.new.empty?.should be_truthy
      Asciicrystal::Reader.new.eof?.should be_truthy
    end

    it "next_line_empty? should return true with empty data" do
      Asciicrystal::Reader.new.next_line_empty?.should be_truthy
    end

    it "peek_line should return nil with empty data" do
      Asciicrystal::Reader.new.peek_line.should be_nil
    end

    it "peek_lines should return empty Array with empty data" do
      Asciicrystal::Reader.new.peek_lines(1).empty?.should be_truthy
    end

    it "read_line should return nil with empty data" do
      Asciicrystal::Reader.new.read_line.should be_nil
    end

    it "read_lines should return empty Array with empty data" do
      Asciicrystal::Reader.new.read_lines.empty?.should be_truthy
    end
  end
  context "With data" do
    ...
    it "next_line_empty? should return true if next line is blank" do
      reader = Asciicrystal::Reader.new(["", "second line"])
      reader.next_line_empty?.should be_truthy
    end

    it "peek_line should return nil if reader is empty" do
      Asciicrystal::Reader.new([] of String).peek_line.should be_nil
    end
    ...
    it "unshift puts line onto Reader as next line to read" do
      reader = Asciicrystal::Reader.new(TestHelpers::SAMPLE_DATA, nil, {:normalize => true})
    ...
    it "skip_blank_lines should skip blank lines" do
      reader = Asciicrystal::Reader.new(["", ""].concat(TestHelpers::SAMPLE_DATA))
  ...
  context "Include Directive" do
    it "should replace include directive with link macro in default safe mode" do
      ...
      doc = Asciicrystal.load(input, {"parse" => "false"})
    ...
    it "should not add role to link macro used to replace include directive in compat mode" do
      input = "include::include-file.adoc[]"
      doc = Asciicrystal.load(input, {"parse" => "false", "attributes" => "compat-mode"})
      reader = doc.reader.not_nil!
    ...
    it "should replace include directive that references missing file with message" do
      ...
      doc.blocks[0].as(Asciicrystal::Block).lines[0].should eq("Unresolved directive in <stdin> - include::fixtures/no-such-file.adoc[]")
    end
    ...
    it "escaped include directive is left unprocessed" do
      ...
      reader = Asciicrystal::PreprocessorReader.new(doc, input, nil, {:normalize => true})
  ...
  context "Include Stack" do
    it "push_include method should return reader" do
      reader = TestHelpers.empty_document.reader.not_nil!.as(Asciicrystal::PreprocessorReader)
    ...
    it "push_include method should put lines on top of stack" do
      lines = ["a", "b", "c"]
      doc = Asciicrystal.load(lines.join("\n"))
      reader = doc.reader.not_nil!.as(Asciicrystal::PreprocessorReader)
      append_lines = ["one", "two", "three"]
  ...
  context "Front Matter" do
    ...
    it "should skip front matter if specified by skip-front-matter attribute" do
      ...
      input = "---\n#{front_matter}\n---\n= Document Title"
      doc = Asciicrystal.load(input, {"parse" => "false", "attributes" => "skip-front-matter"})
      reader = doc.reader.not_nil!
  ...
  describe "#initialize" do
    ...
    it "creates a reader with a string cursor" do
      reader = Asciicrystal::Reader.new(["line 1"], "/tmp/test.adoc")
      reader.file.should eq("/tmp/test.adoc")
    end
  ...
  describe "#advance" do
    it "advances to the next line" do
      reader = Asciicrystal::Reader.new(["line 1", "line 2"])
  ...
  describe "#next_line_empty?" do
    it "returns true when next line is empty" do
      reader = Asciicrystal::Reader.new(["", "line 2"])
  ...
  describe "#read_lines_until" do
    ...
    it "skips line comments when requested" do
      reader = Asciicrystal::Reader.new(["line 1", "// comment", "line 2", "----"])
  ...
  describe "#replace_next_line" do
    it "replaces the next line" do
      reader = Asciicrystal::Reader.new(["old line", "line 2"])
  ...
  describe "#skip_comment_lines" do
    it "skips single-line comments" do
      reader = Asciicrystal::Reader.new(["// comment 1", "// comment 2", "line 1"])
      reader.skip_comment_lines
      reader.peek_line.should eq("line 1")
    end

    it "skips block comments" do
      reader = Asciicrystal::Reader.new(["////", "comment", "////", "line 1"])
      reader.skip_comment_lines
      reader.peek_line.should eq("line 1")
    end
  end
  ...
  describe "#unshift_lines" do
    it "adds lines to the top of the reader" do
      reader = Asciicrystal::Reader.new(["line 3"])
...
describe Asciicrystal::PreprocessorReader do
  describe "#initialize" do
    it "creates a preprocessor reader" do
      doc = Asciicrystal::Document.new
      reader = Asciicrystal::PreprocessorReader.new(doc, ["line 1"])
      reader.should be_a(Asciicrystal::PreprocessorReader)
    end
  end
  ...
  describe "#preprocess_conditional_directive" do
    it "processes ifdef with defined attribute" do
      doc = Asciicrystal::Document.new
      doc.attributes["backend"] = "html5"
      reader = Asciicrystal::PreprocessorReader.new(doc, [
        "ifdef::backend[]",
        "Backend is defined",
        "endif::[]",
        "After endif",
  ...
  describe "empty? and eof? for PreprocessorReader" do
    it "returns true when no lines remain" do
      doc = Asciicrystal::Document.new
      reader = Asciicrystal::PreprocessorReader.new(doc, [] of String)
      reader.empty?.should be_true
      reader.eof?.should be_true
    end
    ...
    it "returns true after include stack is exhausted" do
      doc = Asciicrystal::Document.new(safe: Asciicrystal::SafeMode::UNSAFE)
      reader = Asciicrystal::PreprocessorReader.new(doc, ["root"])
      reader.read_line
...
describe Asciicrystal::Cursor do
  describe "#initialize" do
    ...
    it "creates a cursor without file" do
      cursor = Asciicrystal::Cursor.new(nil)
    ...
    it "creates a cursor with custom lineno" do
      cursor = Asciicrystal::Cursor.new("/tmp/test.adoc", lineno: 10)
      cursor.lineno.should eq(10)
    end

    it "creates a cursor with custom dir and path" do
      cursor = Asciicrystal::Cursor.new("/tmp/test.adoc", dir: "/custom", path: "custom.adoc")
  ...
  describe "#line_info" do
    it "returns formatted line info" do
      cursor = Asciicrystal::Cursor.new("/tmp/test.adoc", lineno: 42)
  ...
  describe "#to_source_location" do
    it "converts to a SourceLocation" do
      cursor = Asciicrystal::Cursor.new("/tmp/test.adoc", lineno: 5)
      loc = cursor.to_source_location
      loc.should be_a(Asciicrystal::SourceLocation)
  ...
  describe "#to_s" do
    it "returns the line_info string" do
      cursor = Asciicrystal::Cursor.new("/tmp/test.adoc", lineno: 3)
