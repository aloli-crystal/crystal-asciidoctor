...
describe Asciicrystal::Document do
  describe "#initialize" do
    ...
    it "creates a document with custom safe mode" do
      ...
      doc.safe.should eq(Asciicrystal::SafeMode::UNSAFE)
    end

    it "creates a document with custom backend" do
      doc = Asciicrystal::Document.new(backend: "docbook5")
    ...
    it "creates a document with sourcemap enabled" do
      doc = Asciicrystal::Document.new(sourcemap: true)
  ...
  describe "#basebackend?" do
    it "returns true when backend starts with base" do
      doc = Asciicrystal::Document.new(backend: "html5")
  ...
  describe "#create_converter" do
    ...
    it "creates a DocBook5Converter for docbook5 backend" do
      ...
      converter.should be_a(Asciicrystal::Converter::DocBook5Converter)
    end

    it "creates a ManPageConverter for manpage backend" do
      doc = Asciicrystal::Document.new
      converter = doc.create_converter("manpage")
      converter.should be_a(Asciicrystal::Converter::ManPageConverter)
    end
  ...
  describe "#docinfo" do
    it "returns empty string when safe mode is SECURE" do
      doc = Asciicrystal::Document.new(safe: Asciicrystal::SafeMode::SECURE)
  ...
  describe "#first_section" do
    ...
    it "returns the first section" do
      doc = Asciicrystal::Document.new
      doc << Asciicrystal::Block.new(doc, :paragraph)
    ...
    it "returns the header if set" do
      doc = Asciicrystal::Document.new
      header = Asciicrystal::Section.new(doc)
      header.title = "Header"
      doc.header = header
  ...
  describe "#footnotes?" do
    ...
    it "returns true when footnotes exist" do
      doc = Asciicrystal::Document.new
      doc.catalog.footnotes << Asciicrystal::Document::Footnote.new(index: 1, id: "fn1", text: "A note")
  ...
  describe "#header?" do
    ...
    it "returns true when header is set" do
      doc = Asciicrystal::Document.new
      doc.header = Asciicrystal::Section.new(doc)
  ...
  describe "#nested?" do
    ...
    it "returns true for a nested document" do
      parent = Asciicrystal::Document.new
      child = Asciicrystal::Document.new(parent_document: parent)
  ...
  describe "#update_doctype_attributes" do
    it "updates doctype and related attributes" do
      doc = Asciicrystal::Document.new(doctype: "article")
  ...
  describe "Asciicrystal::Document::AttributeEntry" do
    it "stores name and value" do
      entry = Asciicrystal::Document::AttributeEntry.new("author", "John Doe")
    ...
    it "negates when value is nil" do
      entry = Asciicrystal::Document::AttributeEntry.new("toc", nil)
      entry.negate.should be_true
    end

    it "saves to attributes hash" do
      attrs = {} of String => String
      entry = Asciicrystal::Document::AttributeEntry.new("key", "value")
      entry.save_to(attrs)
    ...
    it "removes from attributes hash when negated" do
      attrs = {"key" => "value"}
      entry = Asciicrystal::Document::AttributeEntry.new("key", nil)
      entry.save_to(attrs)
      attrs.has_key?("key").should be_false
    end
  end

  describe "Asciicrystal::Document::Footnote" do
    it "stores index, id, and text" do
      fn = Asciicrystal::Document::Footnote.new(index: 1, id: "fn1", text: "A footnote")
  ...
  describe "Asciicrystal::Document::ImageReference" do
    it "stores target and imagesdir" do
      ref = Asciicrystal::Document::ImageReference.new("image.png", "images")
  ...
  describe "Asciicrystal::Document::Title" do
    it "parses a simple title" do
      title = Asciicrystal::Document::Title.new("My Document")
    ...
    it "parses a title with subtitle" do
      title = Asciicrystal::Document::Title.new("Main Title: Subtitle Here")
    ...
    it "uses the last separator for subtitle split" do
      title = Asciicrystal::Document::Title.new("Part One: Chapter: Details")
      title.main.should eq("Part One: Chapter")
      title.subtitle.should eq("Details")
    end

    it "reports subtitle?" do
      title_with = Asciicrystal::Document::Title.new("Main: Sub")
      title_with.subtitle?.should be_true

      title_without = Asciicrystal::Document::Title.new("Simple Title")
      title_without.subtitle?.should be_false
    end
  end

  describe "Asciicrystal::Catalog" do
    it "initializes with empty collections" do
      catalog = Asciicrystal::Catalog.new
  ...
  context "Document Title" do
    ...
    it "should set doctype to book when specified" do
      doc = Asciicrystal.load("", {"doctype" => "book"})
      doc.doctype.should eq("book")
    end
    ...
    it "should set backend to docbook5 when specified" do
      doc = Asciicrystal.load("", {"backend" => "docbook5"})
      doc.backend.should eq("docbook5")
    end
    ...
    it "should set attribute via API" do
      doc = Asciicrystal.load("content", {"foo" => "bar"})
      doc.attr("foo").should eq("bar")
    end

    it "should have correct content model" do
      doc = Asciicrystal.load("content")
      doc.content_model.should eq(Asciicrystal::ContentModel::Compound)
    end
