...
describe "End-to-end integration" do
  describe "Asciicrystal.load" do
    it "loads a simple document from a string" do
      source = "= My Document\nAuthor Name\n\nThis is a paragraph."
      doc = Asciicrystal.load(source)
  ...
  describe "Asciicrystal.convert" do
    ...
    it "converts to docbook5 backend" do
      source = "= Document Title\n\nA paragraph."
      result = Asciicrystal.convert(source, {"backend" => "docbook5"})
      result.should be_a(String)
    end

    it "converts to manpage backend" do
      source = "= command(1)\nAuthor\n\n== NAME\n\ncommand - a test command\n\n== SYNOPSIS\n\n*command* [_options_]"
      result = Asciicrystal.convert(source, {"backend" => "manpage"})
      result.should be_a(String)
    end
  end
  ...
  describe "Reader" do
    it "reads lines from a string source" do
      source = "Line 1\nLine 2\nLine 3"
      reader = Asciicrystal::Reader.new(source.split("\n"))
    ...
    it "supports peek and unshift" do
      reader = Asciicrystal::Reader.new(["A", "B", "C"])
  ...
  describe "Parser utility methods" do
    it "detects delimited blocks" do
      result = Asciicrystal::Parser.is_delimited_block?("----")
      result.should_not be_nil
      result.not_nil!.context.should eq(:listing)
    end

    it "detects section titles" do
      result = Asciicrystal::Parser.is_section_title?("== Section Title", nil)
      result.should_not be_nil
    end

    it "detects atx section title level" do
      # Section titles are parsed via the reader, so we test detection
      result = Asciicrystal::Parser.is_section_title?("= Document Title", nil)
      result.should_not be_nil
      result2 = Asciicrystal::Parser.is_section_title?("=== Level 2", nil)
      result2.should_not be_nil
      result3 = Asciicrystal::Parser.is_section_title?("Not a title", nil)
      result3.should be_nil
    end

    it "parses block metadata line with anchor" do
      doc = Asciicrystal::Document.new
      reader = Asciicrystal::Reader.new(["[[my-anchor]]"])
    ...
    it "parses block metadata line with block title" do
      doc = Asciicrystal::Document.new
      reader = Asciicrystal::Reader.new([".My Title"])
      attrs = {} of String => String
      result = Asciicrystal::Parser.parse_block_metadata_line(reader, doc, attrs)
      result.should be_true
  ...
  describe "HTML5 Converter full pipeline" do
    ...
    it "converts a listing block" do
      ...
      block = Asciicrystal::Block.new(
        parent_block: doc,
        context: :listing,
        content_model: Asciicrystal::ContentModel::Verbatim
      )
      block.style = "source"
    ...
    it "converts an admonition block" do
      ...
      block = Asciicrystal::Block.new(
        parent_block: doc,
        context: :admonition,
        content_model: Asciicrystal::ContentModel::Compound
      )
      block.style = "NOTE"
    ...
    it "converts inline quoted text" do
      ...
      emphasis = Asciicrystal::Inline.new(parent_block: doc, context: :quoted, text: "italic", type: :emphasis)
      result = converter.convert_inline_quoted(emphasis)
      result.should contain("<em>italic</em>")

      strong = Asciicrystal::Inline.new(parent_block: doc, context: :quoted, text: "bold", type: :strong)
      result = converter.convert_inline_quoted(strong)
      result.should contain("<strong>bold</strong>")

      mono = Asciicrystal::Inline.new(parent_block: doc, context: :quoted, text: "code", type: :monospaced)
    ...
    it "converts inline anchors" do
      ...
      link = Asciicrystal::Inline.new(parent_block: doc, context: :anchor, text: "Example", type: :link)
  ...
  describe "Rx patterns" do
    it "matches section titles" do
      match = Asciicrystal::AtxSectionTitleRx.match("== Section Title")
      match.should_not be_nil
    end

    it "matches block anchors" do
      match = Asciicrystal::BlockAnchorRx.match("[[my-id]]")
      match.should_not be_nil
    end

    it "matches attribute entries" do
      match = Asciicrystal::AttributeEntryRx.match(":author: John Doe")
      match.should_not be_nil
    end

    it "matches block titles" do
      match = Asciicrystal::BlockTitleRx.match(".My Title")
      match.should_not be_nil
    end
  end
end
